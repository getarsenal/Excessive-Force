 ...
 * Tokyo Tower, Tokyo.
 *
 * TODO(tokyotower) Reference figures, with sources: plan from tools/survey.py, elevation
 * from photographs. Then the proportion table (docs/maps/tokyotower.md) before a number
 * below is typed.
 *
 * What kind of problem it is (cantilever / lattice / shell / mountain), and
 * the twist the player has to find: TODO(tokyotower)
 */

// Scale above life, chosen before the constants; everything derives from it.
const S = 1.0;                       // TODO(tokyotower)
const STONE_FINENESS = 2.0;
// One batter for every face if this stands on a hill; metres in per metre up.
const BATTER = 0.0;

// Real metres, scaled once at the end. TODO(tokyotower): from the survey.
...
export function buildTokyotower(quality) {
  ...
  // TODO(tokyotower) the building. A placeholder so the level loads and the harness runs.
  B.section('body', () => {
...
export function populateTokyotower(g, origin, groundY) {
  ...
  // TODO(tokyotower) post men where the building has a floor under them and a view out.
  for (const sx of [-1, 1]) {
