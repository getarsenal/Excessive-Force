 ...
 *
 * TODO(budapest) Reference figures, with sources: plan from tools/survey.py, elevation
 * from photographs. Then the proportion table (docs/maps/budapest.md) before a number
 * below is typed.
 *
 * What kind of problem it is (cantilever / lattice / shell / mountain), and
 * the twist the player has to find: TODO(budapest)
 */

// Scale above life, chosen before the constants; everything derives from it.
const S = 1.0;                       // TODO(budapest)
const STONE_FINENESS = 2.0;
// One batter for every face if this stands on a hill; metres in per metre up.
const BATTER = 0.0;

// Real metres, scaled once at the end. TODO(budapest): from the survey.
...
export function buildBudapest(quality) {
  ...
  // TODO(budapest) the building. A placeholder so the level loads and the harness runs.
  B.section('body', () => {
...
export function populateBudapest(g, origin, groundY) {
  ...
  // TODO(budapest) post men where the building has a floor under them and a view out.
  for (const sx of [-1, 1]) {
