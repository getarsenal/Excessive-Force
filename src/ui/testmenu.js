    ...
    return [
      ...
      ['debris never freezes in mid-air', () => this._calm(() => {
        ...
        try {
          ...
          // The pressure check is a precondition, not the point — and one
          // level cannot meet it. Four metres of rammed earth and rubble does
          // not drop single stones: a hole in the Potala's wall either holds
          // or takes a whole connected section with it, and a section comes
          // away welded into one body rather than freed stone by stone. So
          // `traits.sheds` says whether the building sheds loose rubble at
          // all, the way `windows`, `river` and `topples` already say what a
          // level is. Where it does, no pressure means the guard below is not
          // being exercised, and that is a regression in itself.
          if (b.level?.traits?.sheds === false) {
            assert(hanging.length === 0,
              `${hanging.length} stones hang with nothing under them: `
              + `${hanging.slice(0, 8).join(', ')}`);
            return `nothing on ${st.key} sheds loose stone; none hanging`;
          }
